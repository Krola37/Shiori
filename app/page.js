...
export default function ShioriUltimateIntegrator() {
  ...
  const addChildFolder = (parentPath) => {
    ...
    if (name && name.trim()) {
      ...
      if (folders.includes(newPath)) return alert("その名前は既に存在します。");
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 text-slate-800 font-sans antialiased text-left" onClick={() => { setContextMenu(null); setShowSuggest(false); }}>
      ...
      {isSidebarOpen && <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-[60] md:hidden" onClick={() => setIsSidebarOpen(false)} />}

      {/* --- SIDEBAR --- */}
      <aside className={`fixed md:relative h-full border-r bg-white flex flex-col shrink-0 shadow-xl z-[70] transition-all duration-300 ${isSidebarOpen ? "w-64 translate-x-0" : "w-0 -translate-x-full md:translate-x-0 md:w-0 overflow-hidden border-none"}`}>
        <div className="p-6 border-b flex justify-between items-center bg-slate-50/30 min-w-[256px]">
          ...
          <button onClick={(e) => { e.stopPropagation(); const n = prompt("新しいフォルダ名?"); if(n && !folders.includes(n)) setFolders([...folders, n]); }} className="w-8 h-8 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center hover:bg-indigo-600 hover:text-white font-bold transition-all">+</button>
        </div>
        ...
        <div className="p-4 border-t bg-slate-50/50 space-y-2 min-w-[256px]">
          ...
          <input type="file" ref={fileInputRef} onChange={(e) => {
            ...
            reader.onload = (ev) => {
              ...
              if (confirm("復元しますか？")) { setBookmarks(data.bookmarks); setFolders(data.folders); }
            };
          ...
          <header className="h-16 flex items-center justify-between gap-4">
            ...
            <div className="flex-1 flex gap-2 md:gap-6 items-center justify-end">
              <div className="hidden lg:flex items-center gap-3">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">表示サイズ</span>
              ...
              <div className="hidden md:flex bg-slate-100 p-1 rounded-full border text-[9px] font-black uppercase">
                <button onClick={() => setSortType("date")} className={`px-3 py-1 rounded-full ${sortType === "date" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400"}`}>日付</button>
                <button onClick={() => setSortType("title")} className={`px-3 py-1 rounded-full ${sortType === "title" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400"}`}>名前</button>
              </div>
              ...
              <button onClick={() => {setIsEditMode(!isEditMode); setSelectedIds([]);}} className={`text-[10px] font-black px-4 py-2 rounded-full border transition-all ${isEditMode ? "bg-red-500 text-white border-red-500 shadow-lg" : "text-slate-400 hover:border-indigo-400"}`}>一括整理</button>
            </div>
          </header>

          {!isEditMode ? (
            <div className="pb-6 pt-2 px-2 md:px-0">
              <form onSubmit={handleAddBookmark} className="max-w-5xl mx-auto flex flex-col gap-3 bg-white/80 p-3 md:p-5 rounded-2xl md:rounded-[2.5rem] shadow-xl border border-white backdrop-blur-md">
                ...
                <div className="flex flex-wrap md:flex-nowrap gap-4 relative">
                  <input type="text" placeholder="メモ" value={memo} onChange={e => setMemo(e.target.value)} className="flex-1 min-w-[150px] p-2 text-xs border-b border-slate-50 outline-none" />
                  <input type="text" placeholder="#タグ" value={tags} onChange={e => setTags(e.target.value)} className="w-32 p-2 text-xs border-b border-slate-50 outline-none" />
                  ...
                  <button type="submit" disabled={isFetching} className="w-full md:w-auto bg-indigo-600 text-white px-8 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 shadow-lg disabled:opacity-30 transition-all">{isFetching ? "取得中..." : "追加"}</button>
                </div>
              </form>
            </div>
          ) : (
            <div className="py-6 bg-indigo-50/80 backdrop-blur-md flex justify-center gap-6 animate-in slide-in-from-top rounded-b-3xl mx-4 shadow-inner">
              <span className="text-xs font-black text-indigo-600 self-center uppercase">{selectedIds.length} SELECTED</span>
              <button onClick={() => {const d = prompt("移動先フォルダ名?"); if(d && folders.includes(d)) {setBookmarks(bs=>bs.map(b=>selectedIds.includes(b.id)?{...b, folder:d}:b)); setSelectedIds([]); setIsEditMode(false);}}} className="bg-white border px-6 py-2 rounded-xl text-[10px] font-black uppercase shadow-sm">一括移動</button>
              <button onClick={() => {if(confirm("削除しますか?")) {setBookmarks(bs=>bs.filter(b=>!selectedIds.includes(b.id))); setSelectedIds([]); setIsEditMode(false);}}} className="bg-red-500 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg">一括削除</button>
            </div>
          )}
        </div>
            ...
            {filteredAndSortedBookmarks.map((bm) => (
              <div key={bm.id} onClick={() => isEditMode && (selectedIds.includes(bm.id) ? setSelectedIds(selectedIds.filter(i => i !== bm.id)) : setSelectedIds([...selectedIds, bm.id]))} onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); setContextMenu({x: e.pageX, y: e.pageY, item: bm, type: 'bookmark'}); }}
                ...
                <div className="w-24 h-16 md:w-full md:aspect-video rounded-xl overflow-hidden bg-slate-100 border border-slate-50 shrink-0 shadow-inner">
                  {bm.ogImage ? <img src={bm.ogImage} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-[8px] text-slate-200 font-black uppercase tracking-widest">No Image</div>}
                </div>
                <div className="flex-1 flex flex-col justify-center md:mt-4 overflow-hidden">
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">{bm.tags?.map(t => <span key={t} className="text-[8px] font-black text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-full">#{t}</span>)}</div>
                </div>
      ...
      {/* --- CONTEXT MENU & MODALS --- */}
      {contextMenu && (
        <div className="fixed z-[100] bg-white shadow-2xl border border-slate-100 rounded-xl overflow-hidden py-1 w-48 text-[10px] font-black uppercase tracking-widest animate-in fade-in" style={{ top: contextMenu.y, left: contextMenu.x }} onClick={e => e.stopPropagation()}>
          {contextMenu.type === 'bookmark' ? (
            <>
              <div className="px-5 py-3 hover:bg-indigo-50 cursor-pointer" onClick={() => { navigator.clipboard.writeText(contextMenu.item.url); setContextMenu(null); }}>コピー</div>
              <div className="px-5 py-3 hover:bg-indigo-50 cursor-pointer" onClick={() => { setEditingBookmark(contextMenu.item); setContextMenu(null); }}>編集</div>
            ...
            <>
              ...
              <div className="px-5 py-3 hover:bg-indigo-50 cursor-pointer text-indigo-600" onClick={() => addChildFolder(contextMenu.item)}>子フォルダ追加</div>
              {contextMenu.item !== "未分類" && <div className="px-5 py-3 hover:bg-red-50 text-red-500 cursor-pointer border-t" onClick={() => { if(confirm("フォルダを削除?")) setFolders(fs=>fs.filter(f=>f!==contextMenu.item && !f.startsWith(contextMenu.item+"/"))); setContextMenu(null); }}>削除</div>}
            </>
          )}
        </div>
      )}
      ...
      {editingBookmark && (
        <div className="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6" onClick={() => setEditingBookmark(null)}>
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl" onClick={e => e.stopPropagation()}>
            <h2 className="font-black mb-6 text-xl tracking-tighter">✏️ ブックマークの編集</h2>
            <div className="space-y-4 text-[10px] font-black text-slate-400 uppercase">
              <div>URL<input className="w-full mt-1 px-4 py-2 bg-slate-50 rounded-xl outline-none text-sm font-medium" value={editingBookmark.url} onChange={e => setEditingBookmark({...editingBookmark, url: e.target.value})} /></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>タグ<input className="w-full mt-1 px-4 py-2 bg-slate-50 rounded-xl outline-none text-xs font-medium" value={editingBookmark.tags?.join(", ")} onChange={e => setEditingBookmark({...editingBookmark, tags: e.target.value.split(",").map(t => t.trim())})} /></div>
              </div>
            </div>
